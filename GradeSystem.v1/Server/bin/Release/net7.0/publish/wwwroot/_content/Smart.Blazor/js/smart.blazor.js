		...
		instances[id].addEventListener(eventName, function (event) {
			try {
				...
				if (normalizedEvent.detail) {
					if (normalizedEvent.detail.row) {
						normalizedEvent.detail.row = normalizedEvent.detail.row.id;
					}

					if (normalizedEvent.detail.column) {
						normalizedEvent.detail.column = normalizedEvent.detail.column.dataField;
					}

					if (normalizedEvent.detail.columns && !normalizedEvent.detail.nodeName) {
                      normalizedEvent.detail.columns = normalizedEvent.detail.columns.map(column => {
                        return column.dataField
                      });
                    }

          
                    if (normalizedEvent.detail.cell) {
						normalizedEvent.detail.cell = {
							id: normalizedEvent.detail.cell.row.id,
							dataField: normalizedEvent.detail.cell.column.dataField,
							value: normalizedEvent.detail.cell.value
						}

						normalizedEvent.detail.value = normalizedEvent.detail.cell.value;
					}
                }

				setTimeout(() => {
					dotNetObject[that.dotNetInvoke](callback, stringify_object(normalizedEvent));
				});
			}
			catch (err) {
				console.error(err);
			...
			if (instances[id]) {
				if (instances[id].isRendered) {
					...
					if (Array.isArray(result)) {
						if (result[0] && result[0] instanceof window.Smart.Grid.Row) {
							const resultRows = result.map((row) => {
								return row.data;
							});

							return resultRows;
						}
						if (result[0] && result[0] instanceof window.Smart.Grid.Column) {
							const resultColumns = result.map((column) => {
								return column.dataField;
							});

							return resultColumns;
						}
						if (result[0] && result[0] instanceof window.Smart.Grid.Cell) {
							const resultCells = result.map((cell) => {
								return cell.value;
							});
